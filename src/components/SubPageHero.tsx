...
export default function SubPageHero({ title, subtitle, badge, image, position = "bg-center" }: SubPageHeroProps) {
  return (
    <section className="relative h-[85vh] w-full overflow-hidden flex flex-col justify-center px-6 md:px-12 lg:px-24">
      {/* Background Image with Zoom Animation */}
      <motion.div
        initial={{ scale: 1.1 }}
        animate={{ scale: 1 }}
        transition={{ duration: 10, ease: "linear" }}
        className={`absolute inset-0 z-0 bg-cover ${position} bg-no-repeat`}
