...
const DocumentUpload: React.FC<DocumentUploadProps> = ({ uploadedFiles, setUploadedFiles, onNext, onBack }) => {
  ...
  const docTypes = [
    "Drawings", "RFC (8120-10)", "Statement of Conformity (8130-9)", "Inspection plan", 
    "Test plan", "Supplier certs", "Photos", "Calibration records", "NCRs/deviations"
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {docTypes.map((doc) => (
          <div 
            ...
            className={`p-6 border rounded-sm cursor-pointer transition-all flex flex-col items-center justify-center gap-4 text-center group ${
          ...
          >
            {uploadedFiles.includes(doc) ? (
              ...
              <Upload className="w-8 h-8 text-white/20 group-hover:text-gold transition-colors" />
            )}
            <div>
              <p className="text-[10px] font-bold tracking-widest uppercase mb-1">{doc}</p>
              <p className="text-[9px] text-white/40 uppercase">Required Evidence</p>
            </div>
